...
class CeiLoginService {
  ...
  async getToken() {
    switch (this._options.loginOptions.strategy) {
      ...
      default:
        throw CeiCrawlerError(
          CeiErrorTypes.INVALID_LOGIN_STRATEGY,
          `Invalid login strategy: ${this._options.strategy}`
        );
    }
  }
  ...
  async _getTokenByUserResolve() {
    ...
    do {
      try {
        await CeiUtils.retry(
          async () => {
            await homePage.waitForNavigation({ timeout: 1000 });
          },
          2,
          1000,
          true
        );
        await homePage.click(".cabecalho .btn-primary");
        console.log(
          await this._getMainPage(browser),
          "await this._getMainPage(browser);"
        );
    ...
    try {
      ...
      await CeiUtils.retry(
        async () => {
          await mainPage.focus("#DOC_INPUT");
        },
        10,
        1000,
        true
      ...
      await CeiUtils.retry(
        async () => {
          await mainPage.click("#Btn_CONTINUE");
        },
        8,
        301
      );
      await CeiUtils.retry(
        async () => {
          await mainPage.focus("#PASS_INPUT");
        },
        10,
        1000,
        true
      ...
      console.log(
        "Failed trying to fill user info. Will wait for the user to resolve the login himself"
      );
    }

    await mainPage.waitForFunction(
      () => {
        return document.querySelector(".saudacao-investidor") !== null;
      },
      { timeout: 0 }
    );
      ...
      for (const p of await browser.pages()) {
        if (
          p
            .url()
            .indexOf(
              "b3investidor.b2clogin.com/b3Investidor.onmicrosoft.com/oauth2/v2.0/authorize"
            ) !== -1
        )
          return p;
      }
